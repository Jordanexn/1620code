let horseName = "bibble";
console.log(horseName)
horseName = "barbie";
console.log(horseName);
let horseAge = 7;
console.log(horseAge);


let isHorseInside = true; 
console.log(isHorseInside);
let isHorseOutside = false; 
console.log(isHorseOutside);

// lesson-4 code
console.log(typeof "888");
console.log(typeof true);
console.log( typeof false);
console.log(typeof horseName);
console.log(typeof horseAge);

const STABLE_MONTHLY_FEE = 888; // uppercase is preffernce for consts to make them easier to find 
console.log(STABLE_MONTHLY_FEE);

console.log(STABLE_MONTHLY_FEE * 3);
console.log(STABLE_MONTHLY_FEE - 444);
console.log("bye" + "  Felica");
console.log("bye ");


console.log("my horse is iridescent");
console.log("the stable monthly fee is " + STABLE_MONTHLY_FEE); // concatenation 
console.log(`My horse's name is +${horseName}" and costs ${STABLE_MONTHLY_FEE}`);


// LAbb 4 

let horseNickname = "bib" 
console.log(horseNickname);
console.log(`this is my horse ${horseName} but you can call him ${horseNickname}`);
console.log( `it costs  ${STABLE_MONTHLY_FEE} dollars a month to keep him in my stable`);
console.log("we are currently running a promotion where a 10% discount is applied to the purchase of a three month stay");
console.log("you would be saving" + (STABLE_MONTHLY_FEE*3* 0.1));



// Lab 5a//








//LESSON 6//
//Horse = [Bibble, Binkie , Boots]

console.log()
console.log()


let horseNames =[horseName, Binkie, Boots, Brioche];
let horseages = [horseAge, 5, 9];
let horseLocations = [isHorseInside, false, true];
let crazyArray = [ ]
console.log(horseNames.length) 
console.log(horseNames);
horseNames.push("Brioche")
console.log(horseNames);
horseNames.push(Binkie)














